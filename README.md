...
`BareMetalHAL::` namespace. Dynamic memory doesn't work that way, and
...
### Why the example has three files
...
- **`src/avr/MemoryHAL.cpp`** - not example code. This is the library
  file every consumer's build must compile and link (see above) - the
  example's `build.sh` does this the same way a real consuming
  project's build script would.
- **`consumer_no_include.cpp`** - a regression test, not a pattern to
  ...
  development (see `MemoryHAL.h`'s contract entry above).
