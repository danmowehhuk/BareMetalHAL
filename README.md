...
- `void* operator new(size_t size)`, `void* operator new[](size_t size)`,
  ...
  needs it.
- **This category also has a `.cpp` file.** The four operators above
  live in `MemoryHAL.cpp`, not `MemoryHAL.h` - at `-Os`, GCC fully
  inlines an `inline`-qualified operator new/delete at every call site
  and emits no out-of-line definition anywhere, so a translation unit
  that uses `new`/`delete[]` but doesn't itself include `MemoryHAL.h`
  (e.g. a consumer whose own facade header never pulls in
  `BareMetalHAL.h` directly) would have nothing to link against -
  confirmed empirically. So `MemoryHAL`'s contract spans two files:
  `MemoryHAL.h` for `freeMemory()`, `MemoryHAL.cpp` for the operators.
  Same requirement as `TimingHAL.h`/`.cpp` below - a consuming build
  must compile and link `src/<platform>/MemoryHAL.cpp` itself, adding
  it to the include path is not enough. See
  `examples/dynamic-memory-basic-avr/build.sh` for a worked example,
  including a second translation unit
  (`consumer_no_include.cpp`) that uses `new`/`delete[]` without
  including `BareMetalHAL.h` or `MemoryHAL.h` at all, proving the
  cross-TU link actually works end-to-end.
