...
"$AVRGXX" -std=gnu++11 -Wall -Wextra -Os -DNO_ARDUINO -DHAL_AVR -DF_CPU=16000000UL -mmcu=atmega2560 \
  -I "$DIR/../../src" \
  "$DIR/gpio-basic-avr.cpp" -o "$BUILD_DIR/gpio-basic-avr.elf"

"$AVROBJCOPY" -O ihex -R .eeprom "$BUILD_DIR/gpio-basic-avr.elf" "$BUILD_DIR/gpio-basic-avr.hex"

echo "Built $BUILD_DIR/gpio-basic-avr.hex"

if $SIM_MODE; then
  HEX="$BUILD_DIR/gpio-basic-avr.hex"
