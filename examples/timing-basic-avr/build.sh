...
#
# Mirrors examples/gpio-basic-avr/build.sh's shape - bare avr-g++ build,
# no arduino-cli, no Arduino core.
...
"$AVRGXX" -std=gnu++11 -Wall -Wextra -Os -DNO_ARDUINO -DHAL_AVR -DF_CPU=16000000UL -mmcu=atmega2560 \
  -I "$DIR/../../src" \
  "$DIR/timing-basic-avr.cpp" "$DIR/../../src/avr/TimingHAL.cpp" \
  -o "$BUILD_DIR/timing-basic-avr.elf"

"$AVROBJCOPY" -O ihex -R .eeprom "$BUILD_DIR/timing-basic-avr.elf" "$BUILD_DIR/timing-basic-avr.hex"

echo "Built $BUILD_DIR/timing-basic-avr.hex"

if $SIM_MODE; then
  HEX="$BUILD_DIR/timing-basic-avr.hex"
