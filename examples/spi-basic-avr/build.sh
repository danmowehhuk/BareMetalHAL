...
#
# Compile/link smoke test for SpiHAL - proves it links cleanly. Mirrors
# examples/gpio-basic-avr/build.sh - same toolchain-discovery and -s
...
"$AVRGXX" -std=gnu++11 -Wall -Wextra -Os -DNO_ARDUINO -DHAL_AVR -DF_CPU=16000000UL -mmcu=atmega2560 \
  -I "$DIR/../../src" \
  "$DIR/spi-basic-avr.cpp" -o "$BUILD_DIR/spi-basic-avr.elf"

"$AVROBJCOPY" -O ihex -R .eeprom "$BUILD_DIR/spi-basic-avr.elf" "$BUILD_DIR/spi-basic-avr.hex"

echo "Built $BUILD_DIR/spi-basic-avr.hex"

if $SIM_MODE; then
  HEX="$BUILD_DIR/spi-basic-avr.hex"
