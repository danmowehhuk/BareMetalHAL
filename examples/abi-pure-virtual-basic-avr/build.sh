...
find_avr_tool() {
  ...
  local search_roots=(
    ...
    "/opt/homebrew/opt/avr-gcc" "/opt/homebrew/Cellar/avr-gcc"
    "/usr/local/opt/avr-gcc" "/usr/local/Cellar/avr-gcc"
    "/opt/local" "/usr/local/avr" "/opt/avr" "/usr/avr"
  )
...
"$AVRGXX" -std=gnu++11 -Wall -Wextra -Os -DNO_ARDUINO -DHAL_AVR -DF_CPU=16000000UL -mmcu=atmega2560 \
  -I "$DIR/../../src" \
  "$DIR/abi-pure-virtual-basic-avr.cpp" "$DIR/../../src/avr/MemoryHAL.cpp" \
  -o "$BUILD_DIR/abi-pure-virtual-basic-avr.elf"

"$AVROBJCOPY" -O ihex -R .eeprom "$BUILD_DIR/abi-pure-virtual-basic-avr.elf" "$BUILD_DIR/abi-pure-virtual-basic-avr.hex"
echo "Built $BUILD_DIR/abi-pure-virtual-basic-avr.hex"

if $SIM_MODE; then
  HEX="$BUILD_DIR/abi-pure-virtual-basic-avr.hex"
